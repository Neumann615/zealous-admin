...
export default function Hot() {
  ...
  // 列表查询参数
  const [listQuery, setListQuery] = useState<RecommendProductQueryParam>({
    pageNum: 1,
    pageSize: 10,
  })
  // 人气推荐列表数据
  ...
  // 表格中被选中的行
  const [selectedRows, setSelectedRows] = useState<SmsHomeRecommendProduct[]>([])
  ...
  const getList = async () => {
    setListLoading(true)
    try {
      const res = await getHomeRecommendProductListAPI(listQuery)
  ...
  // 处理表格选中变化
  const handleSelectionChange = (selectedRowKeys: React.Key[], selectedRows: SmsHomeRecommendProduct[]) => {
    setSelectedRows(selectedRows)
  }

  // 处理推荐状态变化
  const handleRecommendStatusChange = async (row: SmsHomeRecommendProduct, checked: boolean) => {
    modal.confirm({
      title: '提示',
      content: '是否要修改推荐状态?',
      onOk: async () => {
        try {
          await homeRecommendProductUpdateRecommendStatusAPI({
            ids: String(row.id),
  ...
  // 处理删除
  const handleDelete = (row: SmsHomeRecommendProduct) => {
    modal.confirm({
      title: '提示',
      content: '是否要删除该推荐?',
      onOk: async () => {
        try {
          await homeRecommendProductDeleteByIdsAPI({ ids: String(row.id) })
  ...
  // 编辑排序
  const handleEditSort = (row: SmsHomeRecommendProduct) => {
  ...
  const handleUpdateSort = async () => {
    ...
    modal.confirm({
      ...
      onOk: async () => {
        try {
          await homeRecommendProductUpdateSortByIdAPI(sortDialogData)
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      align: 'center' as const,
      render: (recommendStatus: number, row: SmsHomeRecommendProduct) => (
        <Switch
    ...
    },
    {
      ...
      width: 160,
      align: 'center' as const,
      render: (_: any, row: SmsHomeRecommendProduct) => formatRecommendStatus(row.recommendStatus!),
    },
    {
      title: '操作',
      key: 'actions',
      width: 180,
      align: 'center' as const,
      render: (_: any, row: SmsHomeRecommendProduct) => (
        <Space>
