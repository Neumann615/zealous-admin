...
import {
  getHomeNewProductListAPI,
  homeNewProductCreateAPI,
  homeNewProductDeleteByIdsAPI,
  homeNewProductUpdateRecommendStatusAPI,
  homeNewProductUpdateSortByIdAPI,
...
export default function New() {
  ...
  // 列表查询参数
  const [listQuery, setListQuery] = useState<NewProductQueryParam>({
    pageNum: 1,
    pageSize: 10,
  })
  // 新品列表数据
  const [list, setList] = useState<SmsHomeNewProduct[]>([])
  // 总条数
  ...
  // 表格中被选中的行
  const [selectedRows, setSelectedRows] = useState<SmsHomeNewProduct[]>([])
  ...
  const getList = async () => {
    setListLoading(true)
    try {
      const res = await getHomeNewProductListAPI(listQuery)
  ...
  // 处理表格选中变化
  const handleSelectionChange = (selectedRowKeys: React.Key[], selectedRows: SmsHomeNewProduct[]) => {
    setSelectedRows(selectedRows)
  }

  // 处理推荐状态变化
  const handleRecommendStatusChange = async (row: SmsHomeNewProduct, checked: boolean) => {
    modal.confirm({
      title: '提示',
      content: '是否要修改推荐状态?',
      onOk: async () => {
        try {
          await homeNewProductUpdateRecommendStatusAPI({
            ids: String(row.id),
  ...
  // 处理删除
  const handleDelete = (row: SmsHomeNewProduct) => {
    modal.confirm({
      title: '提示',
      content: '是否要删除该推荐?',
      onOk: async () => {
        try {
          await homeNewProductDeleteByIdsAPI({ ids: String(row.id) })
  ...
  // 编辑排序
  const handleEditSort = (row: SmsHomeNewProduct) => {
  ...
  const handleUpdateSort = async () => {
    ...
    modal.confirm({
      ...
      onOk: async () => {
        try {
          await homeNewProductUpdateSortByIdAPI(sortDialogData)
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      align: 'center' as const,
      render: (recommendStatus: number, row: SmsHomeNewProduct) => (
        <Switch
    ...
    },
    {
      ...
      width: 160,
      align: 'center' as const,
      render: (_: any, row: SmsHomeNewProduct) => formatRecommendStatus(row.recommendStatus!),
    },
    {
      title: '操作',
      key: 'actions',
      width: 180,
      align: 'center' as const,
      render: (_: any, row: SmsHomeNewProduct) => (
        <Space>
        ...
        <Form layout="inline">
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px 24px' }}>
            <Form.Item label="商品名称：">
              <Input
                value={listQuery.productName}
                onChange={e => setListQuery({ ...listQuery, productName: e.target.value })}
                placeholder="商品名称"
                style={{ width: 200 }}
              />
            </Form.Item>
