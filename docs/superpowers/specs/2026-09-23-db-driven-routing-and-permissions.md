...
- ~~监控、元数据、表单页面只做了**接口级**强制，页面内按钮尚未逐个包 `useHasPermission`~~ → 已于同日落地：三模块的增删改与状态开关全部按权限标识显隐 / 降级只读，`e2e/permission.spec.ts` 增只读角色用例。
- ~~菜单管理不支持拖拽排序，`sort` 仍是手填数字~~ → 已落地：`@dnd-kit/react` 同级行拖拽换位（按父节点分组），新增节点自动追加到同级末尾，表单不再暴露 `sort` 手填项。
- ~~目录 `type` 由「有无子节点」一次性推导，之后不会自动重算~~ → 已落地：`menu.service` 在子节点新增 / 删除 / 移动后自动重算父级类型（有路由型子节点为目录，只剩按钮或空则回菜单），`menu.service.test.ts` 覆盖三例。
- ~~权限标识是自由文本，重命名后需要同步 `permission.ts` 的路由表~~ → 已落地：`GET /menu/permissions` 从路由表导出字典，菜单配置改为可搜索下拉（编辑旧值仍可显示）；`menu-permission-consistency.test.ts` 双向校验「路由表 ⇄ 菜单种子」无死标识。
