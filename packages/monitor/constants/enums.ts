import type { AlertRuleMeta } from '../contracts/monitor'
...
export const ERROR_RANGE_TYPES = ['WINDOW_ERROR', 'PROMISE_ERROR']
...
/** 日志页时间快捷档位（毫秒），'' 表示自定义 */
export const LOG_TIME_TYPE_OPTIONS: Array<{ value: number | '', label: string }> = [
...
/** 统计页时间快捷档位（毫秒） */
export const STATS_TIME_TYPE_OPTIONS: Array<{ value: number | '', label: string }> = [
  { value: 3_600_000, label: '最近1小时' },
...
export const CHANNEL_TYPE_OPTIONS = [
  ...
  { value: 'feishu', label: '飞书自定义机器人' },
]

export const TEMPLATE_OPTIONS = [
  { value: 'TEXT_CARD', label: 'TEXT_CARD（文本卡片）' },
  { value: 'MARKDOWN', label: 'MARKDOWN（纯文本）' },
]

/** 预警规则目录：与服务端 ALERT_RULE_CATALOG 保持一致 */
...
export const ALERT_FIELD_LABELS: Record<string, string> = {
  enabled: '启用',
  minDurationMs: '慢门槛(ms)',
  windowMinutes: '窗口(分钟)',
  minCount: '次数下限',
  avgMs: '平均耗时阈值(ms)',
  maxMs: '最大耗时阈值(ms)',
...
export const LONG_WINDOW_NOTE = '窗口超过 24 小时：按天粒度聚合，分位数（P95 等）为直方图近似值。'
export const TRUNCATED_NOTE = '数据量超单窗口上限，部分指标为截断样本口径。'

export function rangeTypeLabel(value?: string | null): string {
  return RANGE_TYPE_OPTIONS.find(item => item.value === value)?.label ?? (value || '-')
}

export function alertTypeLabel(value?: string | null): string {
  return ALERT_TYPE_OPTIONS.find(item => item.value === value)?.label ?? (value || '-')
}

export function perfStageLabel(value: string): string {
  return PERF_STAGE_OPTIONS.find(item => item.value === value)?.label ?? value
}