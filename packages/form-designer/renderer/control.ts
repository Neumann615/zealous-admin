import type { ControlCondition, ControlRule } from '../types/schema'
 ...
 */
function matchCondition(condition: ControlCondition, values: Record<string, any>): boolean {
  if (typeof condition.field !== 'string' || !condition.field)
    return false
  const value = getByPathName(values, condition.field)
  switch (condition.operator ?? 'eq') {
    ...
    case 'gt':
      return compareValues(value, condition.value) !== null && compareValues(value, condition.value)! > 0
    case 'gte':
      return compareValues(value, condition.value) !== null && compareValues(value, condition.value)! >= 0
    case 'lt':
      return compareValues(value, condition.value) !== null && compareValues(value, condition.value)! < 0
...
function matchRule(rule: ControlRule, values: Record<string, any>): boolean {
  const conditions: ControlCondition[] = rule.conditions === undefined
    ? [{ field: rule.field ?? '', operator: rule.operator, value: rule.value }]
    : rule.conditions
  if (!conditions.length)
    return false
  return conditions.every(condition => matchCondition(condition, values))
}
...
export function evalControl(rules: ControlRule[] | undefined, values: Record<string, any>): EffectiveState {
  ...
  for (const rule of rules ?? []) {
    if (!rule || (!rule.conditions?.length && typeof rule.field !== 'string'))
      continue
